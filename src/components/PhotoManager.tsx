...
const PhotoManager = () => {
  ...
  const addPhoto = (girlId: string, persona: 'gentle' | 'bold', url: string) => {
    if (!url.trim()) return;
      ...
      prev.map((girl) =>
        girl.id === girlId
          ? {
              ...
              photos: {
                ...girl.photos,
                [persona]: [...girl.photos[persona], url],
              },
            }
          : girl
      )
  ...
  const removePhoto = (girlId: string, persona: 'gentle' | 'bold', index: number) => {
    setPhotos((prev) =>
      prev.map((girl) =>
        girl.id === girlId
          ? {
              ...
              photos: {
                ...girl.photos,
                [persona]: girl.photos[persona].filter((_, i) => i !== index),
              },
            }
          : girl
      )
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Добавьте фото для каждой девушки по персонам (нежная/дерзкая)
            </p>
          </div>
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-3">
                  ...
                  <Badge variant="secondary">
                    {girl.photos.gentle.length + girl.photos.bold.length} фото
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="gentle">
                  <TabsList className="grid w-full grid-cols-2 mb-4">
                    <TabsTrigger value="gentle" className="flex items-center gap-2">
                      ...
                      Нежная ({girl.photos.gentle.length})
                    </TabsTrigger>
                    <TabsTrigger value="bold" className="flex items-center gap-2">
                      <Icon name="Flame" size={16} />
                      Дерзкая ({girl.photos.bold.length})
                    </TabsTrigger>
                  </TabsList>

                  {(['gentle', 'bold'] as const).map((persona) => (
                    <TabsContent key={persona} value={persona} className="space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {girl.photos[persona].map((photo, index) => (
                          <div key={index} className="relative group">
                            ...
                            <Button
                              ...
                              onClick={() => removePhoto(girl.id, persona, index)}
                            >
                      ...
                      <div className="flex gap-2">
                        <Input
                          ...
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') {
                              addPhoto(girl.id, persona, newPhotoUrl);
                            }
                          }}
                        />
                        <Button onClick={() => addPhoto(girl.id, persona, newPhotoUrl)}>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>
                    • Каждая девушка имеет две персоны: <strong>Нежную</strong> и{' '}
                    <strong>Дерзкую</strong>
                  </li>
                  <li>• Фото отправляются по очереди при запросе пользователя</li>
                  <li>• После последнего фото список начинается сначала</li>
                  <li>• Персона выбирается пользователем в чате (уровень 1+)</li>
                </ul>
              </div>
            </div>
          </CardContent>
